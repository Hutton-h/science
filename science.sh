...
[ -z "${sopt+x}" ] || sop=yes
[ -z "${warp+x}" ] || wap=yes
# 一键全协议预设：all=y 自动开启8大协议+订阅（Argo需额外指定agn和agk）
[ -z "${all+x}" ] || { vlp=yes; vmp=yes; vmag=yes; vwp=yes; vmag=yes; hyp=yes; tup=yes; xhp=yes; vxp=yes; anp=yes; ssp=yes; sub=y; vmag=yes; }
...
echo "（Argo需CF托管域名+隧道Token，无则自动用临时隧道trycloudflare.com）"
